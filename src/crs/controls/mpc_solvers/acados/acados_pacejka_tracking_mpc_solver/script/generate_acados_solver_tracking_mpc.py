...
from pacejka_model_tracking_mpc import pacejka_model_tracking_mpc
...
def build_acados_solver(N: int, Ts: float, acados_source_path: str, model):
    ...
    filename = "c_generated_code/acados_pacejka_tracking_mpc_solver_config.json"
    acados_solver = AcadosOcpSolver(ocp, json_file=filename)



if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Create a new Acados tracking MPC solver for the pacejka model')
    parser.add_argument('--config', type=str, help='Name of the configuration file', default="solver_tracking_mpc.yaml")
    ...
    with open(f"../config/{args.config}") as f:
        ...
        def loadModel():
            return pacejka_model_tracking_mpc(cfg["model_bounds"])
